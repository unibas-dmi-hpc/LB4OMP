...
#include "kmp_cov_reward.h"
#include "../agents/kmp_agent.h"

double CovReward::reward(LoopData *stats, Agent *agent) {
#if (_RL_DEBUG > 1)
    std::cout << "[CovReward::reward] Getting reward ..." << std::endl;
#endif
    double reward_signal = stats->cCOV;
    double low = agent->get_low();
    double high = agent->get_high();
    agent->set_reward_val(reward_signal);

#if (_RL_DEBUG > 0)
    std::cout << "[CovReward::reward] High: " << high << ", Low: " << low << ", Reward: " << reward_signal
